...
const readBar = (page: Page) => page.evaluate(() => {
  const btns = [...document.querySelectorAll<HTMLElement>(".qc-phead button, .qc-verbs-inert button")];
  return btns.map((b) => ({
...
test("§2 — Mark closed, Export and Delete take the bar's ink", async ({ page }) => {
  ...
  for (const b of live) console.log(`  ${b.ghost ? "ghost" : "live "} "${b.label}" → ${b.colour}${b.disabled ? " [disabled]" : ""}${b.ariaDisabled ? " [aria-disabled]" : ""}`);
  ...
  /* ⚠️ THE SAME INK AS THE BAR'S OTHER ENABLED VERB — asserted against Nudge rather than a hex, so
     the claim is "they match" rather than "they are #5c5a58". */
  const nudge = live.find((b) => !b.ghost && b.label.startsWith("Nudge") && !b.ariaDisabled);
  expect(nudge, "no enabled Nudge on this query to compare the ink against").toBeTruthy();
  for (const b of found) expect(b.colour, `"${b.label}" does not match the bar's enabled ink`).toBe(nudge!.colour);
