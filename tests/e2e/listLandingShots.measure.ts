 ...
 *
 * ⚠️ THIS IS NOT `listShots.measure.ts`, AND THE NAMES ARE ONE CHARACTER APART FOR A REASON WORTH
 * KNOWING: I wrote this file over that one. `listShots` belongs to the list-port round, shoots a
 * THIRD contract (`todo-tasklist-contract.html`) at 1440 and 390 into `reports/list-port/`, and was
 * restored from `bcd7af63` the moment `git diff --cached --name-status` reported `M` where a new
 * file should read `A`. A heredoc does not ask whether the path is taken; the staged-status letter
 * is what asked.
 *
