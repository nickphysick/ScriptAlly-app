...
for (const [side, url, app] of [["REF", pathToFileURL(REF).href, false], ["APP", APP + "/dashboard", true]]) {
  ...
  /* ⚠️ EVERY WHOLE PIXEL OF X, NOT 40 SAMPLES (v30, Phase 6). A sliver from a mitered band stroke at
     a sharp peak is one or two pixels wide, and a 40-point sample steps straight over it. */
  const above = [];
  ...
  const yAt = (x) => {
    for (let i = 1; i < g.samples.length; i++) {
      const [ax, ay] = g.samples[i - 1], [bx, by] = g.samples[i];
      if (bx !== ax && x >= Math.min(ax, bx) && x <= Math.max(ax, bx)) return ay + ((by - ay) * (x - ax)) / (bx - ax);
    }
    return null;
  };
  for (let x = Math.ceil(firstX); x <= Math.floor(lastX); x++) {
    if (Math.abs(x - lastX) < 12 || Math.abs(x - firstX) < 12) continue;   // both markers
    const y = yAt(x);
    if (y == null) continue;
    for (const dy of [1.2, 2.2]) above.push([x, y - (g.lineW / 2) - dy]);
  }
  ...
  /* (c) THE FADE (v30, Phase 5) — the SAME band at 15% and 85% of the plot's height. One band,
     because the stack changes identity with height and comparing a pink at the top with a sand at
     the foot measures the palette rather than the fade. */
  const fadeAt = await page.evaluate((arg) => {
    const host = document.querySelector('[data-probe="plot"]');
    const svg = host && (host.tagName.toLowerCase() === "svg" ? host : host.querySelector("svg"));
    if (!svg) return null;
    const hb = host.getBoundingClientRect(), sb = svg.getBoundingClientRect();
    const vb = (svg.getAttribute("viewBox") || "0 0 1 1").split(/\s+/).map(Number);
    const toHostY = (uy) => (sb.top - hb.top) + (uy - vb[1]) * (sb.height / vb[3]);
    const lines = [...svg.querySelectorAll("line")].filter((l) => !l.closest("defs"));
    const axis = lines.find((l) => (l.getAttribute("class") || "").indexOf("axis0") >= 0)
      || lines.filter((l) => (l.getAttribute("stroke") || "") === "#d8cec2")[0];
    if (!axis) return null;
    const zero = toHostY(Number(axis.getAttribute("y1"))), top = toHostY(vb[1]);
    return { x: Math.round(hb.width * arg.fx), top, zero };
  }, { fx: 0.5 });
  /* a whole COLUMN, so the band boundaries are visible in the profile rather than assumed */
  const COL = 40;
  const fadePts = [];
  if (fadeAt) for (let k = 0; k < COL; k++) {
    fadePts.push([fadeAt.x, fadeAt.top + ((fadeAt.zero - fadeAt.top) * k) / (COL - 1)]);
  }
  const px = await page.evaluate(readPixels, { url: dataUrl, points: [...above, ...below, ...fadePts] });
  report[side] = { g, aboveN: above.length, belowN: below.length, px, above, below, fadeAt };
  await page.close();
}
await browser.close();
...
for (const side of ["REF", "APP"]) {
  ...
  if (r.fadeAt) {
    const fp = r.px.points.slice(r.aboveN + r.belowN);
    const dist = (c) => { if (!c) return null; const [R, G, B] = c.split(",").map(Number);
      return Math.round(Math.sqrt((R - 253) ** 2 + (G - 251) ** 2 + (B - 247) ** 2) * 10) / 10; };
    console.log("\n  — THE FADE · a column at x=" + r.fadeAt.x + ", plot top to baseline —");
    const rows = fp.map((c, i) => ({ f: i / (fp.length - 1), c, d: dist(c) })).filter((q) => q.c);
    const ink = rows.filter((q) => q.d > 2);
    for (const q of rows.filter((_, i) => i % 4 === 0)) {
      console.log("    " + (q.f * 100).toFixed(0).padStart(3) + "% of plot  rgb(" + q.c + ")  distance " + q.d);
    }
    if (ink.length >= 4) {
      const band = ink;                      /* the painted span, top of stack to baseline */
      const at = (t) => band[Math.min(band.length - 1, Math.round(t * (band.length - 1)))];
      const hi = at(0.15), lo = at(0.85);
      console.log("    within the PAINTED span:  15% d=" + hi.d + "   85% d=" + lo.d
        + "   ratio " + (hi.d ? Math.round((lo.d / hi.d) * 100) / 100 : "n/a") + "   (gate: < 0.5)");
    }
  }
  const belowCols = r.px.points.slice(r.aboveN, r.aboveN + r.belowN).filter(Boolean);
  const tally2 = {};
