 ...
 * materials →" (soft-pink, ink Playfair, no border-top) opens the materials manager. Ported from the
 * mockup .qlist; colours are theme tokens.
 */
...
export const MaterialsRail: React.FC<MaterialsRailProps> = ({ versions, onCreate, onManage }) => (
  <aside className="pkgrail">
    <style>{`
      ...
      .pkgrail .qlist-head h2 { font-family:${FONT_SERIF}; font-size:18px; font-weight:600; color:var(--ink); margin:0; }
      ...
      .pkgrail .add { width:26px; height:26px; border-radius:7px; background:var(--pink); border:1px solid var(--pink-b); color:var(--burg); display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0; font-size:15px; line-height:1; padding:0; }
      .pkgrail .add:hover { background:var(--pink-h); }
      .pkgrail .qlist-foot { padding:12px; flex-shrink:0; }
      .pkgrail .manage { width:100%; font-family:${FONT_SERIF}; font-size:14px; font-weight:600; color:var(--ink); background:var(--pink); border:1px solid var(--pink-b); border-radius:9px; padding:10px; cursor:pointer; }
      .pkgrail .manage:hover { background:var(--pink-h); }
