...
/* ⚠️ THE THREE STAGE CONSTANTS MOVED WITH THE ROW THAT READS THEM — `STAGE_BADGE_PX`,
   `STAGE_MIN_DAYS` and `STAGE_NARROW_DAYS` are now declared in `shared/timeline/TimelineBoard`,
   beside the gate that applies them. Left here they would have been three more constants with
   one reference each, which is the state seven of their neighbours are already in. */
...
/* ⚠️ THE BOARD'S LEAVES NOW LIVE IN `shared/timeline` — the card, the markers and the action
   ...
   `outerHTML` diff at three widths in `reports/calendar-extract-dom/`). The derivations that FEED
...
export const TodoCalendarPage: React.FC<TodoCalendarPageProps> = ({ onNavigate, onNavigatePath = () => {} }) => {
  ...
  /* ⚠️ `CardPayload` IS THE BOARD'S NOW — imported above. It was declared here and in the
     board after the move, which is two statements of one shape waiting to disagree. */
        ...
        >
          ...
          <div className="tl-cal tl-board" data-dens={density}>
            ...
            <TplZone className="tl-zone" hem={false} label={range.label}>
              <TimelineBoard
                range={range} today={today} todayAt={todayAt} months={months}
                dateLabels={dateLabels} dayDate={dayDate}
                board={board} drawnGroups={drawnGroups} rows={rows} rowNumber={rowNumber}
                barsByRow={barsByRow} sparse={sparse}
                collapsedGroups={collapsedGroups} toggleGroup={toggleGroup}
                sel={sel} setSel={setSel} hoverSeg={hoverSeg} pickSeg={pickSeg}
                openCardOver={openCardOver} cardAt={cardAt} closeCard={closeCard}
                nudgeCountFor={nudgeCountFor}
                wrapRef={wrapRef} onLaneMove={onLaneMove} clearCross={clearCross}
                dragWindow={dragWindow} onRowsOver={onRowsOver} onRowsOut={onRowsOut}
                cross={cross} actToast={actToast}
                onNavigatePath={onNavigatePath}
                pick={pick} requestAction={requestAction} setDragTask={setDragTask} endDrag={endDrag}
              />
            </TplZone>
            </div>
          </div>
