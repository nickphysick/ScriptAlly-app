...
import { PersonalGenre } from "../../lib/genres";
import { PromoTile, visibleTiles } from "../../lib/promoTiles";
import { Band, bandFor, markerClass, markerFraction, axisFor } from "../../lib/wordCountBands";
...
export const ManuscriptPromos: React.FC<ManuscriptPromosProps> = ({
  user, manuscript, versions, onDismiss, onVersions, onPackages,
}) => {
  const tiles = visibleTiles({ user, manuscriptId: manuscript?.id ?? null, versions });
      ...
      {tiles.includes("wordcount") && (
        <article className="mpr-tile">
          ...
          <div className="mpr-info">
            <div className="wcb">
              {rows.map(({ band, yours }) => (
                <div className="wcb-row" key={band.label + String(yours)}>
                  <span className="wcb-label">{yours ? "Yours" : band.label}</span>
