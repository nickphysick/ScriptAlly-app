...
export type GroupBy = "attention" | "status" | "package" | "none";
...
export const GROUP_BY_OPTIONS: readonly { key: GroupBy; label: string }[] = [
  { key: "attention", label: "Attention" }, { key: "status", label: "Status" },
  { key: "package", label: "Submission package" }, { key: "none", label: "Nothing" },
];
...
export const sortDiffers = (v: CalView): boolean => v.groupBy !== CAL_DEFAULT.groupBy || v.sortBy !== CAL_DEFAULT.sortBy || v.asc !== CAL_DEFAULT.asc;
export const anyDiffers = (v: CalView): boolean => filterDiffers(v) || sortDiffers(v);
...
export interface CalGroup { key: string; label: string; count: number; rows: EyeRow[] }
