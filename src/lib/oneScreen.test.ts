...
describe("weeklyLedger", () => {
  it("spans first-send week to the current week, empty weeks included", () => {
    const ledger = weeklyLedger([q({ dateSent: daysAgo(21) }), q({ dateSent: daysAgo(0) })], NOW);
    expect(ledger).toHaveLength(4); // weeks of 13, 20, 27 Jul and 3 Aug
    expect(ledger[1].sent).toBe(0); // the quiet week is a row, not a gap
    expect(ledger[0].label).toBe("13 Jul");
  });

  it("⚠️ RECONCILES BY CONSTRUCTION: Δactive === sent − closed, every week (§3)", () => {
    const queries = [
      q({ dateSent: daysAgo(21) }),
      q({ dateSent: daysAgo(20), status: QueryStatus.REJECTED, lastStatusChange: daysAgo(8) }),
      q({ dateSent: daysAgo(13) }),
      q({ dateSent: daysAgo(6), status: QueryStatus.NO_RESPONSE, lastStatusChange: daysAgo(2) }),
      q({ dateSent: daysAgo(1) }),
    ];
    const ledger = weeklyLedger(queries, NOW);
    for (let i = 1; i < ledger.length; i++) {
      expect(ledger[i].active - ledger[i - 1].active, ledger[i].label)
        .toBe(ledger[i].sent - ledger[i].closed);
    }
    // and week 0 reconciles against an empty board
    expect(ledger[0].active).toBe(ledger[0].sent - ledger[0].closed);
  });

  it("a close can never precede its own send", () => {
    // a terminal doc whose only usable stamp is BEFORE dateSent (bad legacy data) clamps to the send
    const ledger = weeklyLedger(
      [q({ dateSent: daysAgo(3), status: QueryStatus.REJECTED, lastStatusChange: daysAgo(10) })],
      NOW,
    );
    const total = ledger.reduce((a, w) => a + w.closed, 0);
    expect(total).toBe(1);
    expect(ledger[ledger.length - 1].active).toBe(0); // sent then closed — never negative
  });

  it("no sends → an empty ledger, not a single hallucinated week", () => {
    expect(weeklyLedger([q({})], NOW)).toEqual([]);
  });
});

describe("ledgerView + rangeChip", () => {
  const ledger = weeklyLedger(
    Array.from({ length: 12 }, (_, i) => q({ dateSent: daysAgo(i * 7) })),
    NOW,
  );

  it("slices the trailing window; 'all' is everything", () => {
    expect(ledgerView(ledger, "8")).toHaveLength(8);
    expect(ledgerView(ledger, "all")).toHaveLength(ledger.length);
  });
