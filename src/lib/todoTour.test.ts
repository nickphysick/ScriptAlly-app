...
describe("TOUR_STOPS — five stops, copy verbatim, Done on the last", () => {
  it("is exactly the five ref stops in order", () => {
    expect(TOUR_STOPS.map((s) => s.sel)).toEqual([
      ".tdb-postits",
      "#tdb-lane-do",
      "#tdb-lane-do .tdb-pill",
      ".tdb-fab",
      ".tdb-ribbon .tdb-btn-pri",
    ]);
  });
  it("copy snapshot (verbatim from the ref)", () => {
    expect(TOUR_STOPS.map((s) => [s.h, s.p])).toEqual([
      ...
      ["Today lives in the corner.", "Your committed list and everything you’ve done today, struck through as you go. Watch the ring fill."],
