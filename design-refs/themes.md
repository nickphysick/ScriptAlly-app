...
- **Tokens live in the theme scope**, `.t-capp` in `index.css`, never the base sheet: `--qc-ref-rim` (1px dashed burgundy at .28), `--qc-ref-rule` (1px solid burgundy at .13), `--qc-ref-plate` (`#f7f2e9`, the wish-list pull-quote), and the band trio `--qc-ref-band-a/-b/-rule`. Consumed by a `.t-capp`-scoped block in `f12.css`.
- **⚠️ THE HEADER TAKES THE DASHBOARD'S SAGE BAND — the same family, the same stops, not a new green.** The house rule is **sage for a container's header, pink reserved for a surface asking something of you**; a reference panel asks nothing. The trio mirrors `.os-ahead` in `oneScreen.css`, which holds them as literals — if that sheet retunes, these follow. Two container headers in one app must not be two different greens.
- **⚠️ AND A FILLED BAND FORCES THE RIM ONTO AN OVERLAY.** A child background meeting the container's top edge paints over an inset rim — the fault the dashboard hit the moment its own bands landed. The rim is drawn on `::after` (`position:absolute; inset:0; border-radius:inherit; pointer-events:none`) so it sits **above** every child; the host takes `position:relative` and `border:0` so `inset:0` means exactly the panel's outline. **The dash survived the move** — verified in the browser at 3×, even through both corner arcs. The band relies on the panel's own `overflow:hidden` for its corners and must never round itself: two radii that can disagree is one radius too many.
