...
- **`ToDoPage.tsx:750` contradicts the `paneCard` hold rule at `ToDoPage.tsx:694`.** The effect's `narrowed ? dockable[0].key` deliberately jumps the pane to the first visible card when a search narrows; the comment eight lines up says "a rail with nothing in it and a pane still showing your card is the CORRECT pair". Two documented intentions in one file, with the effect winning. Not touched — which of the two is wanted is a product call.
- **`ToDoPage.tsx:615` `hkTop` is built and never read.** An array of `{kind:"group"|"card"}` assembled every render from `vGroups` and `staleSorted`, with a head comment describing it as the bulk bar's and the keyboard walker's shared order. Both of those retired; nothing references it. Dead, and cheap to delete.
- **The rail's list and the pane's queue are two different derivations.** The rail draws `railGroups()` off `boardCols`; the pane walks `dockAllCards()` off `board`. On the harness account the rail showed 12 rows while the pane's position line read `TASK 1 OF 23`. Item 4 added the sweep cards to the queue and removed their members, which narrows the gap but does not close it — the two lists are still assembled separately, against the standing rule that the pane walks exactly what you were looking at.
- **The `fix` journey is still unwalked, and now for a better reason.** Every data-quality gap on the harness account belongs to a group, so the only `fix` row is a cohort and correctly opens the sweep. The per-agent journey needs an individual fix card, which this account cannot produce — its derived step stack remains unit-locked only.
