...
## PHASE 5 RE-ISSUE — housekeeping at scale, reconciled (stale individual · mute surfaces · LIVE assisted fill + provenance)

**Context:** Phase 5 first shipped as `b3885a8`; this pack re-issue (post-re-layouts) adds the deltas. **Gates:** app `tsc` + functions `tsc` clean · `vite build` OK · Vitest **1013** green (+11). Engine guard: `db.tsx`/`recordResponse`/`queryPrimaryAction` untouched; `App.tsx` untouched → no PaintMode; the `firestore.rules` diff is exactly the two additive `fieldSources` hunks.

**Step 0 (all confirmed):** rule keys = `agentDataQualityNeeds` (responseTime — the `===0` quick-add stub only; absence = a deliberate "Unknown", not a gap — /materials/mswl) + `no_response_close`; agent fields = `mswlNotes`, `materialsWanted`, `responseTimeWeeks` **+ `noResponseMeansNo`** (now included in the reply-window batch row, mirroring the drawer's DataQualityBody); **every agent is already included** — the engine's `agents.forEach` emits dq tasks unconditionally, queried or not; **Pro entitlement hook = `isProUser`** (`src/lib/suggestComps.ts`, re-exported via `assistFill.ts`) with the canonical slate PRO pill + `onNavigate("plans")` upgrade route; **Cloud Function entry point = `assistAgentData`** (europe-west2, `functions/src/assistAgentData.ts`, server-side Pro gate, writes nothing).

### The deltas
- **Stale queries stay INDIVIDUAL — never batched.** `groupHousekeeping` now emits dq groups only; `no_response_close` cards render as individual landscape cards in the Housekeeping lane (each opens the drawer's proven three-way: close / still waiting / stop asking). The batch drawer's stale branch is deleted. Lane/tile count = `hkGapCount(groups) + staleCards.length` (the mockup's 25 = 12+9 gaps + 4 stale). *(Reconcile note: the board mockup draws a stale pile card; the pack text — written after that mockup — says individual. Text wins; flagged.)*
- **Queried-first + pip.** `HkMember` gains `queried` (from live queries) + `agency`; members sort queried-first (stable) inside each group; the batch row shows a small burgundy pip on queried agents. Unit-locked.
- **Batch drawer, per the pack anatomy:** rows = monogram · name (+pip) · agency · one field; **chip picker** for reply windows (4/6/8/12/16w + an "other" number input) + a "No reply = no" tick; materials ticks; wish-list textarea. **Filled rows dim** (hover restores); a **counter** ("n of m filled") in the header; **✕ skips** a row for the session; **"Nothing here is required — fill what you know"** stated. Save writes only filled rows through the existing `updateAgent`, per-row error isolation, and the toast now carries a **single "Undo all"** (previous values captured pre-save and restored through the same `updateAgent`; closest-restorable caveat: a previously-ABSENT `noResponseMeansNo` restores as `false` — `updateAgent` can't delete fields).
- **Mute — two scopes, plainly stated** ("muting stops the reminder; the gap stays on the profile"): per-row **Mute** (TaskFlag → `MUTED_UNTIL`) · footer **Stop asking** (rule → `mutedTaskRules`) · the group card itself gains **"Never"** (rule-scope, per the pack's card anatomy). **After 2 snoozes a row proactively offers to stop asking** (`card.snoozes >= 2` → the inline gold offer). **Muted items sit behind "n muted — show"** in the drawer header — `mutedMembersForRule` (pure, unit-locked) lists item-muted agents that still have the gap; each has **Unmute** (`upsertTaskFlag(…, { snoozedUntil: null })` — recon-confirmed `null` clears). **Rule-mute recovery** (the trap the pack didn't cover): muted rules show as unmute chips in a strip under the Housekeeping lane header, so "Never"/"Stop asking" is always reversible.
- **Assisted fill — LIVE.** `ASSIST_LIVE` now defaults **true** (the pack asserts Blaze + the API key are in place, superseding the earlier hold). Free → the button carries the canonical **PRO pill** and routes to `/plans` (manual entry stays fully free; no look-up call is made). Pro → `fetchAssistedFill` calls the real callable, **raced against a 25s timeout** (`raceTimeout`, typed `deadline-exceeded` — a hang never blocks the manual path; failures land in a quiet message and manual entry continues). **Honest empties:** agents the look-up couldn't source show "Not found — enter manually" — never fabricated (the function core also drops provenance-less values server-side). **Provenance persists:** the chip reads "✨ Found · {source} · {date} — check before saving", and on Save a found value kept UNEDITED writes `Agent.fieldSources[{field}] = { source, foundAt }` via `withProvenance` (an edited value is the writer's own — saved provenance-free). Unit-locked: live-default, timeout, provenance attach/merge/skip.

### ⚠️ Two prerequisites for Nick (listed, NOT run)
1. **Deploy the function:** `cd functions && npm install && npm run build && firebase deploy --only functions:assistAgentData` (uses the existing `ANTHROPIC_API_KEY` secret; verify the `web_search_20250305` tool string is current). Until this lands, a Pro click fails fast into the graceful "couldn't reach" state.
2. **Deploy firestore.rules** (dev: `firebase deploy --only firestore:rules --config firebase.dev.json --project dev`; prod: `npm run deploy:rules`): the `fieldSources` allowlist + `isValidAgent` clause are in the file but undeployed — **until then, any assisted save that carries provenance is silently DENIED** (the affectedKeys gotcha) and reports as a failed row; manual saves are unaffected. `mutedTaskRules` + `taskFlags` are already in the file (dev-deployed 16 Jul; prod rules still pending).

**Layout jsdom-unverified** — Nick eyeballs: grouped cards + individual stale cards in the lane, the muted-rules strip, the batch drawer (chips, dim, counter, skip, snooze-offer, muted-show/unmute), the free-account PRO pill vs the Pro-account live run, provenance chips, and the Undo-all toast.

**Commit:** `feat(todo): housekeeping re-issue — stale individual, mute surfaces, live assisted fill + provenance` (the pack's canonical message names `b3885a8`; a verbatim reuse would make history ambiguous).
