...
## Query database — "the desk" page treatment
- **DONE on `main`, UNDEPLOYED (hosting-only).** The signature black frame + 10px rail + edge-glint moved OFF the reading pane and onto **one `.qdesk` frame** (`src/components/Queries.tsx`) wrapping the action bar + list/pane split; sidebar + global top nav stay outside it. Inside the desk the **list card** and **reading pane** are now plain white paper cards (`1px #cbc4b6`, radius 12, soft shadow) on a `#f6f1e9` parchment surface. The pane lost its own frame/rail/glint; its status chip inset came down to `top:22`.
- **List rows = `.qrow`** (lifted-page): rounded, no dividers (all `divide-y` stripped), selected lifts (white + shadow + `translateX(2px)` + 3px burgundy `::before` bar), hover `#fffdf9`; reduced-motion drops the transform.
- **Row meta = queried date** ("Queried 14 Mar", year only when not current). Nick's rule: show the **manuscript name too** ONLY when `manuscripts.length > 1 && selectedManuscriptFilter === "All"` (the only cross-manuscript view); otherwise date alone. The relative "X days ago" was **dropped**. (Note: grouped-by-manuscript with "All" still shows both — minor redundancy, group header already names the book.)
...
- Commits `ba9d487` → `008a5dd` → `1f1572a`. Mockup `design-reference/queries-page-desk.html`. **Eyeball on dev** (auth-gated; preview harness can't log in). The dev-only `ReadingPaneLab` Header mock in `App.tsx` still shows the OLD framed pane — known drift, out of scope.
